...
import {CalendarClock, Filter, Search, CheckCircle, ChevronDown, Printer} from 'lucide-react-native';
import {getPromises} from '../../api/billing';
...
const ACCENT: [string, string] = ['#06B6D4', '#0D9488'];
...
export default function PromiseDateReportsScreen() {
  ...
  const [promiseTypePicker, setPromiseTypePicker] = useState(false);
  ...
  const load = useCallback(async (showRefresh = false) => {
    try {
      ...
      setPromises(data as any[]);
    } catch (err: any) {
      Alert.alert('Error', err?.response?.data?.message || 'Failed to load promise records');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const filteredData = useMemo(() => {
    return promiseData.filter(item => {
      ...
      if (promiseType !== 'all' && item.promiseType !== promiseType) return false;
  ...
  }, [promiseData, fromDate, toDate, sublocality, promiseType, reportType, connectionType]);

  const totalReceivable = filteredData.reduce((sum, item) => sum + item.amount, 0);

  const printColumns: PrintColumn<PromiseRecord>[] = [
    {header: '#', render: (_: PromiseRecord, i: number) => i + 1},
    ...
    {header: 'Promise Date', render: item => formatDate(item.promiseDate)},
    ...
    {header: 'Promised By', render: item => item.promisedBy || '—'},
  ];
  ...
  return (
    <ReportLayout
      title="Promise Date Reports"
      subtitle="View and manage promise date records"
      icon={CalendarClock}
      accent={ACCENT}
      ...
      <KpiRow>
        <KpiCard
          ...
          fg="#2563EB"
        />
        <KpiCard label="No. of Defaulters" value={filteredData.length} icon={CalendarClock} bg="#FEF3C7" fg="#D97706" />
      </KpiRow>
          ...
          <View style={styles.filterField}>
            ...
            <TouchableOpacity style={styles.selectBox} onPress={() => setSublocalityPicker(true)} activeOpacity={0.85}>
              <Text style={[styles.selectText, sublocality === 'all' && styles.placeholder]} numberOfLines={1}>
                {sublocality === 'all' ? 'All' : sublocalityOptions.find(o => o.value === sublocality)?.label || sublocality}
              </Text>
              <ChevronDown size={15} color="#6B7280" />
            </TouchableOpacity>
          </View>
          <View style={styles.filterField}>
            <Text style={styles.label}>Promise Type</Text>
            <TouchableOpacity style={styles.selectBox} onPress={() => setPromiseTypePicker(true)} activeOpacity={0.85}>
              <Text style={[styles.selectText, promiseType === 'all' && styles.placeholder]}>
                {PROMISE_TYPE_OPTIONS.find(o => o.value === promiseType)?.label || promiseType}
              </Text>
              <ChevronDown size={15} color="#6B7280" />
            </TouchableOpacity>
          </View>
          <View style={styles.filterField}>
            ...
            <TouchableOpacity style={styles.selectBox} onPress={() => setReportTypePicker(true)} activeOpacity={0.85}>
              <Text style={[styles.selectText, reportType === 'all' && styles.placeholder]}>
                {STATUS_OPTIONS.find(o => o.value === reportType)?.label || reportType}
              </Text>
              <ChevronDown size={15} color="#6B7280" />
            </TouchableOpacity>
          </View>
      ...
      />
      <OptionPickerSheet
        visible={promiseTypePicker}
        title="Select Promise Type"
        options={PROMISE_TYPE_OPTIONS}
        value={promiseType}
        emptyLabel="All"
        onSelect={setPromiseType}
        onClose={() => setPromiseTypePicker(false)}
      />
      <OptionPickerSheet
      ...
      <View style={styles.card}>
        <View style={styles.listHeader}>
          <View style={styles.listHeaderRow}>
            <View style={styles.listHeaderInfo}>
              <Text style={styles.listTitle}>Promise Date History</Text>
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            <CalendarClock size={36} color="#D1D5DB" />
            <Text style={styles.emptyText}>No promise date records found</Text>
            ...
            return (
              <View key={item.key} style={styles.logCard}>
                ...
                <View style={styles.metaGrid}>
                  ...
                  <View style={styles.metaItem}>
                    <Text style={styles.metaLabel}>Promise Date</Text>
                    <Text style={styles.metaValue}>{formatDate(item.promiseDate)}</Text>
                  </View>
                  <View style={styles.metaItem}>
                  ...
                  <View style={styles.metaItem}>
                    <Text style={styles.metaLabel}>Promised By</Text>
                    <Text style={styles.metaValue} numberOfLines={1}>
                      {item.promisedBy || '—'}
                    </Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  printBtnText: {fontSize: 12, fontWeight: '700', color: '#06B6D4'},
