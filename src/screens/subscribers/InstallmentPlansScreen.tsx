...
const emptyForm: Partial<InstallmentPlan> = {
  name: '',
  installments: 1,
...
function PlansDivider() {
  return (
    <View style={styles.heroDivider}>
      <Svg height="2" width="100%">
        <Defs>
          <LinearGradient id="plansHeroGrad" x1="0" y1="0" x2="1" y2="0">
            <Stop offset="0" stopColor="#F43F5E" stopOpacity="1" />
            <Stop offset="0.7" stopColor="#EC4899" stopOpacity="0.6" />
            <Stop offset="1" stopColor="#EC4899" stopOpacity="0" />
          </LinearGradient>
        </Defs>
        <Rect x="0" y="0" width="100%" height="2" fill="url(#plansHeroGrad)" />
      </Svg>
    </View>
  );
}

export default function InstallmentPlansScreen() {
  const nav = useNavigation();
  const drawerStatus = useDrawerStatus();
  const [plans, setPlans] = useState<InstallmentPlan[]>([]);
  const [filtered, setFiltered] = useState<InstallmentPlan[]>([]);
  ...
  const [form, setForm] = useState<Partial<InstallmentPlan>>(emptyForm);
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    try {
      ...
      Alert.alert('Error', 'Failed to load installment plans');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const activePlans = useMemo(
    () => plans.filter(p => p.status === 'active' || p.status === 'Active').length,
    [plans],
  );

  const statCards: {key: string; label: string; value: number; icon: any; gradient: [string, string]}[] = [
    {key: 'total', label: 'Total Plans', value: plans.length, icon: PieChart, gradient: ['#F43F5E', '#DB2777']},
    {key: 'active', label: 'Active Plans', value: activePlans, icon: Clock, gradient: ['#10B981', '#16A34A']},
  ];
      ...
      {text: 'Cancel', style: 'cancel'},
      {
        ...
        onPress: async () => {
          try {
            ...
            const msg = err.response?.data?.message || err.response?.data?.error || 'Failed to delete plan';
  ...
  const setField = (key: keyof InstallmentPlan, value: any) => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload: Partial<InstallmentPlan> = {
        ...form,
        name: form.name.trim(),
        installments: parseInt(String(form.installments), 10) || 1,
        percentageIncrease: parseFloat(String(form.percentageIncrease)) || 0,
      };
      if (editing) {
        await updateInstallmentPlan(editing.id, payload);
      } else {
      ...
      const msg = err.response?.data?.message || err.response?.data?.error || 'Failed to save plan';
      Alert.alert('Error', msg);
    } finally {
      setSaving(false);
    }
  ...
  const renderItem = ({item}: {item: InstallmentPlan}) => {
    return (
      <TouchableOpacity style={styles.card} onPress={() => openEdit(item)}>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoValue} numberOfLines={1}>{item.installments}</Text>
        </View>
        <View style={styles.infoRow}>
          <Text style={styles.infoLabel}>Percentage Increase</Text>
          <Text style={styles.infoValue} numberOfLines={1}>
            {Number(item.percentageIncrease) || 0}%
          </Text>
        </View>
          ...
          <View>
            ...
            <View style={styles.heroHeader}>
              ...
              <View style={styles.heroInfo}>
                <Text style={styles.heroTitle}>Installment Plans</Text>
                <Text style={styles.heroSubtitle}>Create and manage installment plans.</Text>
              </View>
            </View>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>
              {search ? 'Try adjusting your search' : 'Add your first plan'}
            </Text>
          </View>
        }
        ListFooterComponent={
          <View style={styles.pagination}>
            <Text style={styles.paginationInfo}>
              ...
              {Math.min(currentPage * pageSize, filtered.length)} of {filtered.length} plans
            </Text>
        ...
        <KeyboardAvoidingView
          ...
          <View style={styles.formSheet}>
            <View style={styles.formSheetHeader}>
              <View style={styles.formSheetTitleRow}>
                ...
                <Text style={styles.formSheetTitle}>
                  {editing ? 'Edit' : 'Create'} Installment Plan
                </Text>
              </View>
            ...
            <ScrollView contentContainerStyle={styles.formBody} keyboardShouldPersistTaps="handled">
              {formRow('Plan Name', form.name, t => setField('name', t), 'e.g., 6-Month Easy Plan')}
              {formRow('Number of Installments', form.installments, t => setField('installments', t), 'e.g., 6', 'numeric')}
              {formRow('Percentage Increase (%)', form.percentageIncrease, t => setField('percentageIncrease', t), 'e.g., 5', 'numeric')}
              <View style={styles.formActions}>
                ...
                <GradientButton
                  ...
                  {saving ? (
                    ...
                    <Text style={styles.saveBtnText}>{editing ? 'Update' : 'Save Plan'}</Text>
                  )}
                </GradientButton>
              </View>
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  infoLabel: {fontSize: 12, color: '#9CA3AF', width: 140},
