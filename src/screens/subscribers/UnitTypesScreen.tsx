...
import {getUnitTypes, createUnitType, updateUnitType, deleteUnitType} from '../../api/inventory';
...
function UnitTypesDivider() {
  return (
    <View style={styles.heroDivider}>
      <Svg height="2" width="100%">
        <Defs>
          <LinearGradient id="unitTypesHeroGrad" x1="0" y1="0" x2="1" y2="0">
            <Stop offset="0" stopColor="#3B82F6" stopOpacity="1" />
            <Stop offset="0.7" stopColor="#4F46E5" stopOpacity="0.6" />
            <Stop offset="1" stopColor="#4F46E5" stopOpacity="0" />
          </LinearGradient>
        </Defs>
        <Rect x="0" y="0" width="100%" height="2" fill="url(#unitTypesHeroGrad)" />
      </Svg>
    </View>
  );
}
...
export default function UnitTypesScreen() {
  ...
  const [filtered, setFiltered] = useState<UnitType[]>([]);
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    try {
      ...
      const reason =
        ...
        'Failed to load unit types. Check your connection and try again.';
      setError(reason);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const activeCount = items.filter(
    u => (u as any).status === 'active' || (u as any).status === 'Active',
  ).length;
  const categoryCount = new Set(
    items.map(u => (u as any).category).filter(Boolean),
  ).size;

  const statCards: {key: string; label: string; value: string; icon: any; gradient: [string, string]}[] = [
    {key: 'total', label: 'Total Types', value: String(items.length), icon: Ruler, gradient: ['#3B82F6', '#4F46E5']},
    {key: 'active', label: 'Active Types', value: String(activeCount), icon: CheckCircle, gradient: ['#10B981', '#16A34A']},
    {key: 'categories', label: 'Categories', value: String(categoryCount), icon: Scale, gradient: ['#F59E0B', '#EA580C']},
  ];
  ...
  const openEdit = (unitType: UnitType) => {
    setEditing(unitType);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.message || err.response?.data?.error || 'Failed to save unit type';
  ...
  const handleDelete = (unitType: UnitType) => {
    Alert.alert('Delete Unit Type', `Delete ${unitType.name}?`, [
      {text: 'Cancel', style: 'cancel'},
      {
        ...
        onPress: async () => {
          try {
            await deleteUnitType(unitType.id);
            fetchData(false);
          } catch (err: any) {
            const msg =
              err.response?.data?.message || err.response?.data?.error || 'Failed to delete unit type';
            Alert.alert('Error', msg);
          }
        },
      },
  ...
  const renderItem = ({item, index}: {item: UnitType; index: number}) => (
    ...
    <View style={styles.container}>
      <GradientView colors={['#166534', '#22c55e']} style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <Text style={styles.headerTitle}>Unit Types</Text>
        ...
        ListHeaderComponent={
          <View>
            ...
            <View style={styles.heroHeader}>
              <GradientView colors={['#3B82F6', '#4F46E5']} style={styles.heroIconBox}>
                <Ruler size={20} color="#FFFFFF" />
              </GradientView>
              <View style={styles.heroInfo}>
                <Text style={styles.heroTitle}>Unit Types</Text>
                <Text style={styles.heroSubtitle}>
                  Manage units of measurement for products.
                </Text>
              </View>
            </View>
        ...
        ListEmptyComponent={
          error ? (
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyTitle}>Failed to load unit types</Text>
            ...
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyTitle}>No unit types found</Text>
              <Text style={styles.emptyText}>
                {search ? 'Try adjusting your search' : 'Add your first unit type'}
              </Text>
            </View>
          )
        }
        ListFooterComponent={
          <View style={styles.pagination}>
            <Text style={styles.paginationInfo}>
              ...
              {Math.min(currentPage * pageSize, filtered.length)} of {filtered.length} unit types
            </Text>
          ...
          <View style={styles.formSheet}>
            <View style={styles.formSheetHeader}>
              <View style={styles.formSheetTitleRow}>
                <GradientView colors={['#3B82F6', '#4F46E5']} style={styles.formSheetIcon}>
                  <Ruler size={16} color="#FFFFFF" />
                </GradientView>
