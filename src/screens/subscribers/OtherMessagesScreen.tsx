...
function OtherMessagesDivider() {
  return (
    <View style={styles.heroDivider}>
      <Svg height="2" width="100%">
        <Defs>
          <LinearGradient id="otherMessagesHeroGrad" x1="0" y1="0" x2="1" y2="0">
            <Stop offset="0" stopColor="#8B5CF6" stopOpacity="1" />
            <Stop offset="0.7" stopColor="#7C3AED" stopOpacity="0.6" />
            <Stop offset="1" stopColor="#7C3AED" stopOpacity="0" />
          </LinearGradient>
        </Defs>
        <Rect x="0" y="0" width="100%" height="2" fill="url(#otherMessagesHeroGrad)" />
      </Svg>
    </View>
  );
}
...
export default function OtherMessagesScreen() {
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    try {
      ...
      Alert.alert('Error', 'Failed to load other messages');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  }, [sentMessages, connectionMap, title, sublocality, status, type, box, pkg, company, areaName]);
  ...
  const handleDelete = (m: Message) => {
    Alert.alert(
      'Delete Message',
      `Delete the message sent to "${m.name}"? This cannot be undone.`,
      [
          ...
          <View>
            ...
            <View style={styles.heroHeader}>
              <GradientView colors={['#8B5CF6', '#7C3AED']} style={styles.heroIconBox}>
                <MailQuestion size={20} color="#FFFFFF" />
              </GradientView>
              <View style={styles.heroInfo}>
                <Text style={styles.heroTitle}>Other Messages</Text>
                <Text style={styles.heroSubtitle}>View messages sent to subscribers</Text>
              </View>
            </View>

            <OtherMessagesDivider />
        ...
        <View style={styles.sheetOverlay}>
          <View style={styles.sheet}>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Message Preview</Text>
...
const styles = StyleSheet.create({
  ...
  headerCount: {fontSize: 12, color: '#DDD6FE'},
  heroHeader: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  pageNumActive: {backgroundColor: '#8B5CF6', borderColor: '#8B5CF6'},
